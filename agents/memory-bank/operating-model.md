...
- Canonical updates live in `agents/memory-bank/**`; stamp via `node agents/scripts/update-memory-stamp.mjs` after stable changes.
...
- Validation: `npm run agent:finalize` runs formatting, validation, drift, and quality checks before completion.
