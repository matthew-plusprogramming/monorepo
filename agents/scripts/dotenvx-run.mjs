...
const USAGE = `Usage: node agents/scripts/dotenvx-run.mjs <dotenvx args>

Runs dotenvx and emits a hint when missing private key errors occur.

Example
  node agents/scripts/dotenvx-run.mjs run -f .env.production -- vite build --mode production
`;
...
const resolveHintCommand = (repoRoot) => {
  const scriptPath = resolve(repoRoot, 'agents/scripts/sync-worktree-env-keys.mjs');
  const relativePath = relative(process.cwd(), scriptPath);
  if (relativePath === '') {
    return 'node agents/scripts/sync-worktree-env-keys.mjs';
  }
  ...
  if (sawMissingKey) {
    ...
    if (!hasEnvKeys && inWorktree) {
      try {
        ...
        console.error('This looks like a worktree missing .env.keys.');
        console.error(
          'Run: node agents/scripts/sync-worktree-env-keys.mjs',
        );
      }
    } else if (!hasEnvKeys) {
