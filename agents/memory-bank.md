...
repo_git_sha: a6870cb80efd845e950f239550073745a583cad4
---
...
- Always include: `agents/workflows/default.workflow.md`, `agents/memory-bank/project.brief.md`, recent `agents/ephemeral/active.context.md` (use `node agents/scripts/reset-active-context.mjs` to recreate the template if needed).
...
- After each phase, append a reflection to `agents/ephemeral/active.context.md` via the append-memory-entry script; when stable, roll up into an ADR or a relevant stable memory bank file.
