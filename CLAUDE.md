...
The main agent MAY read files under `.claude/traces/` directly, without dispatching an Explore subagent. This is a scoped exception to the delegation-first constraint.

**Boundary**: This exception applies only to automation-generated index files under `.claude/traces/` that summarize source code structure. The criterion: files generated by automation that summarize source code are readable directly; raw source code is not.

**Explicit exclusions**: Source code files, configuration files, log files, environment files, and human/agent-authored files remain off-limits for direct reading by the main agent. This exception does not extend to any path outside `.claude/traces/`.
