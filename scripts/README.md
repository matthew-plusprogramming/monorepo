...
- `node scripts/manage-cdktf-state.mjs cdk output <stack> [--prod]` writes CDK outputs for a specific stack. Omitting `<stack>` in an interactive terminal opens a picker so you can select and confirm one or more stacks; the script runs each output sequentially with the same flags. Append `--` before any extra CDK arguments (for example, `-- --context stage=dev`) to forward them to every selected stack.
